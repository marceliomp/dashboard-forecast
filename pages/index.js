...
+                              deal={deal}
+                              users={users}
+                              stageInfo={getStageInfo(deal.STAGE_ID)}
+                              type="hot"
+                            />
+                          ))}
+                        </div>
...
+          {stageInfo.name}
+        </div>
+        {!isLost && <div style={{ fontSize: '13px', color: '#666' }}>{stageInfo.probability}% probabilidade</div>}
+      </div>
+    </div>
+  );
+};
...
+                          ? '#f8d7da'
+                          : stageData.probability >= 70
+                          ? '#cce5ff'
+                          : stageData.probability >= 40
+                          ? '#fff3cd'
+                          : '#e2e3e5',
+                      color:
+                        stageData.probability === 100
